...
class FeatureRegression(nn.Module):
    def __init__(self, input_nc=512, output_size=6, norm_layer=nn.BatchNorm2d):
        ...
        # Channel reduction layer that will be adjusted if needed
        self.channel_reduction = nn.Sequential(
            nn.Conv2d(1, 512, kernel_size=1),
            norm_layer(512),
            nn.ReLU()
        )
        
        self.conv = nn.Sequential(
            nn.Conv2d(512, 512, kernel_size=4, stride=2, padding=1), norm_layer(512), nn.ReLU(),
            nn.Conv2d(512, 256, kernel_size=4, stride=2, padding=1), norm_layer(256), nn.ReLU(),
            nn.Conv2d(256, 128, kernel_size=3, padding=1), norm_layer(128), nn.ReLU(),
            nn.Conv2d(128, 64, kernel_size=3, padding=1), norm_layer(64), nn.ReLU()
        )
        self.linear = nn.Linear(64 * 3 * 3, output_size)
        self.tanh = nn.Tanh()

    def forward(self, x):
        # If input has multiple channels, pool or compress them down
        b, c, h, w = x.size()
        if c > 512:
            # Reshape to treat spatial locations as features and pool channels
            x = x.view(b, c, h * w)  # [b, c, h*w]
            x = F.adaptive_avg_pool1d(x, h * w // 4)  # Pool channels
            x = x.view(b, -1, h, w)  # Reshape back
            # Further reduce to 512 if needed
            if x.size(1) > 512:
                x = x[:, :512, :, :] + F.adaptive_avg_pool2d(x[:, 512:, :, :], (h, w))[:, :512, :, :]
        
        # Final reduction to 512 channels if needed
        if x.size(1) != 512:
            x = nn.Conv2d(x.size(1), 512, kernel_size=1).to(x.device)(x)
